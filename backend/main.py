...
from tapo import requests
from dotenv import load_dotenv

import asyncio, time, math, datetime, os, threading
...
class Sunlike(object):
    def __init__(self, device: ColorLightHandler):
        self.device = device
        self.version = "Sunlike 0.0.4a"
        self.MAX_STEPS_PER_M = 240 # 1/s
    ...
    while True:
        ...
        try:
            ...
            if now.hour == 11 and now.minute == 00:
                await sunlike.set_config(brightness=1, color_temp=2500) # Set to lowest values, so it starts low
                ...
                await sunlike.device.on() # Make sure to turn on first
                await sunlike.linear_gradient_set_config(time_m=60, end_brightness=75, end_color_temp=2900)
            
            if now.hour == 20 and now.minute == 00:
                await sunlike.linear_gradient_set_config(time_m=55, end_brightness=75, end_color_temp=2500)
                time.sleep(30)
                await sunlike.shift_from_temp_to_hue()
                
            if now.hour == 21 and now.minute == 00:
                await sunlike.linear_gradient_set_config(time_m=55, end_brightness=50, end_hue=25, end_saturation=60)
            
            if now.hour == 22 and now.minute == 00:
                await sunlike.linear_gradient_set_config(time_m=25, end_brightness=25, end_hue=20, end_saturation=60)
                
            if now.hour == 23 and now.minute == 15:
                await sunlike.linear_gradient_set_config(time_m=25, end_brightness=1, end_hue=15, end_saturation=80)
                
            time.sleep(60)
            
        # Except below the sleep, so another chance to trigger event is possible
        except Exception as ex:
            if "connect error" in ex:
                # Refresh device
                device = get_device(client)
                sunlike.device = device
